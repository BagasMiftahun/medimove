...
@section('content')
    <div class="main-content">
        <div class="page-header">
            <h2 class="header-title">Payment Edit</h2>
        ...
        <div class="card">
            <div class="card-body">
                <h4>Payment</h4>
                <div class="m-t-25">
                    <form action="{{ route('payment.update', $payment->id) }}" method="POST" id="form-validation">
                        @csrf
                        @method('PUT')
                        <div class="form-group row">
                            ...
                            <div class="col-md-5">
                                <select class="form-control" id="customer-select" name="order_id" required>
                                    ...
                                    <option value="{{ $order->id }}" {{ $payment->order_id == $order->id ? 'selected' : '' }} data-order-date="{{ $order->order_date }}" data-total-amount="{{ $order->total_amount }}">{{ $order->customer->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-5">
                                <div class="input-affix m-b-10">
                                    ...
                                    <input type="text" class="form-control datepicker-input" id="order-date" value="{{ $payment->order->order_date }}" placeholder="Pick a date" disabled>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="input-group col-md-5">
                                ...
                                <input type="text" class="form-control" id="sub_total" name="sub_total" placeholder="Enter a Sub Total"value="{{ $payment->order->total_amount }}" disabled>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-5">
                                <div class="input-affix m-b-10">
                                    <i class="prefix-icon anticon anticon-calendar"></i>
                                    <input type="text" class="form-control datepicker-input" id="payment-date" name="payment_date" value="{{ $payment->payment_date }}" placeholder="Pick a date" required>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
