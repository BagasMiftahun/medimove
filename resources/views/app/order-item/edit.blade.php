...
@section('title', 'Item Order| Laundry')
...
@section('content')
    <div class="main-content">
        <div class="page-header">
            <h2 class="header-title">Order Item Edit</h2>
        ...
        <div class="card">
            <div class="card-body">
                <h4>Order Item</h4>
                <div class="m-t-25">
                    <form action="{{ route('order-item.update', $item->id) }}" method="POST" id="form-validation">
                        @csrf
                        @method('PUT')
                        <div class="form-group row">
                            ...
                            <div class="col-md-5">
                                <select class="form-control" id="customer-select" name="order_id" required>
                                    ...
                                    <option value="{{ $order->id }}" {{ $item->order_id == $order->id ? 'selected' : '' }} data-order-date="{{ $order->order_date }}">{{ $order->customer->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-5">
                                <div class="input-affix m-b-10">
                                    ...
                                    <input type="text" class="form-control datepicker-input" id="order-date" placeholder="Pick a date" value="{{ $item->order->order_date }}" disabled>
                                </div>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-md-5">
                                <select class="form-control" name="service_id" required>
                                    ...
                                    <option value="{{ $service->id }}" {{ $item->service_id == $service->id ? 'selected' : '' }}>{{ $service->item->name ?? ''}} {{  $service->serviceType->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="input-group col-md-5">
                                ...
                                <input type="text" class="form-control" id="quantity" name="quantity" placeholder="Enter a Quantity" value="{{ $item->quantity }}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="input-group col-md-5">
                                ...
                                <input type="text" class="form-control" id="sub_total" name="sub_total" placeholder="Enter a Sub Total" value="{{ $item->sub_total }}" required disabled>
                            </div>
                        </div>
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            // Set order date when page loads
            var selectedOption = $('#customer-select').find('option:selected');
